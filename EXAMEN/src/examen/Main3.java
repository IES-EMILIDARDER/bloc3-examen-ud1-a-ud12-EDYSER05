...
public class Main3 {
    ...
    public static void main(String[] args) {
        ...
        Function<Vehicle, Vehicle> incrementaPreu1 = v -> {
            v.setPreu(v.getPreu()*1.1);
            return v;
        };
        
        //BIFUNCTION
        BiFunction<Vehicle, Double, Vehicle> incrementaPreu2;
        incrementaPreu2 = (v1,porcentaje) -> { 
            v1.setPreu(v1.getPreu()*porcentaje); 
            return v1; 
        };
        
        //UTILITZAR BIFUNCTION Y MOSTRAR VEHICLES
        vehicles.stream().map(v -> incrementaPreu2.apply(v,1.1)).forEach(v -> System.out.println(v));
    }
}
